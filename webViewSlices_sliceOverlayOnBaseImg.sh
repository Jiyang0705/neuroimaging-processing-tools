...
CNSP_webViewSlices_sliceOverlayOnBaseImg(){
	...
	# in case image name has suffix
	baseImgFilename=`echo $2 | awk -F'.' '{print $1}'`
	overlayImgFilename=`echo $4 | awk -F'.' '{print $1}'`
	...
	${FSLDIR}/bin/pngappend ${outputDir}/${overlayOnBaseImg}_Slice_x1.png \
		...
		${outputDir}/png/${overlayOnBaseImg}_Slices_merged.png

	## remove overlay-on-base slices, and overlay-on-base image
	rm -f ${outputDir}/${overlayOnBaseImg}_Slice_??.png
	rm -f ${outputDir}/${overlayOnBaseImg}.nii*
